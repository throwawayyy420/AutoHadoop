...
echo "      addresses: [$master_ip/24]" >> /etc/netplan/00-installer-config.yaml
...
echo "        addresses: [$master_ip, 8.8.8.8, 8.8.4.4]" >> /etc/netplan/00-installer-config.yaml
...
# edit hostname
sed -i 's/server/master/g' /etc/hostname
reboot